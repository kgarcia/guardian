class MesasController < ApplicationController
  ...
  def check_centro
    @mesa = Mesa.find_by_numero_and_centro_id(params[:mesa][:numero],params[:mesa][:centro_id])
  ...
  private
    ...
    def mesa_params
      params.require(:mesa).permit(:numero, :centro_id, :user_id, :lista, :teodoro, :marquina, :nulo, :votos)
    end
end
